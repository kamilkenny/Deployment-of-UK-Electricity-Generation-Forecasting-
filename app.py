...
try:
    ...
    st.warning("⚠️ Image 'kamil_profile.jpg' not found in repository.")
...
# 4. Model & Scaler Loading (IMPROVED PATHING)
@st.cache_resource
def load_assets():
...
# 🔍 HINDCASTING DATA LOADER (SIMPLIFIED FOR PRE-NAMED COLUMNS)
@st.cache_data
def load_hindcast_data():
    # ⚠️ IMPORTANT: Replace 'kamilkenny' and 'your-repo' with your actual details
    url = "https://raw.githubusercontent.com/kamilkenny/your-repo/main/load_hindcast_data.csv"
    try:
        df_h = pd.read_csv(url)

        # Columns are already DATETIME and GENERATION in your CSV
...
st.divider()
st.subheader("📅 Configure Prediction Horizon")
...
predict_btn = st.button("🚀 Run LSTM Forecast Analysis", type="primary", use_container_width=True)
...
if predict_btn:
    if isinstance(selected_range, tuple) and len(selected_range) == 2:
        ...
        # 🕵️ HINDCASTING VALIDATION (OCT '25 - FEB '26)
        if not df_val.empty:
            st.divider()
            st.subheader("❄️ Winter Hindcasting Validation (Oct 2025 - Feb 2026)")
        ...
        st.error("⚠️ Please select a valid range (start and end date) on the calendar.")
