...
# The existing kamil_profile.png asset is displayed responsively. Landscape
# images are shown wide for readability, while portrait images are kept compact.
# -----------------------------------------------------------------------------
if PROFILE_PATH.exists():
    try:
        ...
        if aspect_ratio >= 1.35:
            image_left, image_main, image_right = st.columns([0.25, 4.5, 0.25])
            with image_main:
                st.image(
                    project_image,
                    width=900,
                    caption="GB electricity generation forecasting · Project visual",
                )
        else:
